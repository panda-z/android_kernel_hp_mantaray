...
{
	volatile long usb_portsc = readl(USB_PORTSC);

	if ((usb_portsc & PORTSC_LS) == PORTSC_LS) {
		atomic_set(&ui->thirdparty_charger, 0);
		return USB_CHG_TYPE__WALLCHARGER;
	} else if ( ((usb_portsc & PORTSC_LS) == (0x02 << 10) ) && \
		(usb_portsc & (1 << 7)) && \
		(usb_portsc & (1 << 2)) && \
		(usb_portsc & (1 << 0)) ) {
		/* It is observed that when a third party charger is
		 * connected, the USB line status is reported as J-state,
		 * port is in suspend state, port is enabled and
		 * device is connected to the port.
		 */
		atomic_set(&ui->thirdparty_charger, 1);
		/* Return the charger type as a known wall
		 * charger type.
		 */
		return USB_CHG_TYPE__WALLCHARGER;
	} else {
...
#define USB_WALLCHARGER_CHG_CURRENT 860 
/* For third party chargers, lets use a
 * conservative current limit */
#define USB_THIRDPARTY_CHARGER_CHG_CURRENT 500
...
static int usb_get_max_power(struct usb_info *ui)
{
	...
	if (temp == USB_CHG_TYPE__WALLCHARGER)
		return atomic_read(&ui->thirdparty_charger) ? \
			USB_THIRDPARTY_CHARGER_CHG_CURRENT : \
...
{
	struct dentry *dent;
	dent = debugfs_create_dir("msm_usb", NULL);
	if (IS_ERR(dent))
		return;
...
int usb_gadget_unregister_driver(struct usb_gadget_driver *driver)
{
	...
	/* temporary workaround: flush all requests before calling unbind */
	flush_all_endpoints(dev); /* flush all current requests */
