...
static ssize_t a6_generic_store(struct device *dev, struct device_attribute *dev_attr, const char *buf,
				size_t count);
static ssize_t a6_generic_show_battery_params(struct device *dev, struct device_attribute *dev_attr, char *buf);
static ssize_t a6_generic_store_battery_params(struct device *dev, struct device_attribute *dev_attr, const char *buf,
				size_t count);
...
static ssize_t a6_valid_battery_show(struct device *dev, struct device_attribute *dev_attr, char *buf);
static ssize_t a6_valid_battery_show_bp(struct device *dev, struct device_attribute *dev_attr, char *buf);

static ssize_t a6_battauth_show(struct device *dev, struct device_attribute *dev_attr, char *buf);
static ssize_t a6_battauth_show_bp(struct device *dev, struct device_attribute *dev_attr, char *buf);
static ssize_t a6_battauth_store(struct device *dev, struct device_attribute *dev_attr, const char *buf, size_t cnt);
static ssize_t a6_battauth_store_bp(struct device *dev, struct device_attribute *dev_attr, const char *buf, size_t cnt);
static ssize_t a6_refresh_battregs_show(struct device *dev,
...
struct device_attribute custom_devattr[] = {
	...
	{{.name = "validate_battery", .mode = S_IRUGO},
	. show = a6_valid_battery_show, .store = NULL},
	{{.name = "battauth", .mode = S_IRUGO | S_IWUGO},
	. show = a6_battauth_show, .store = a6_battauth_store},
	{{.name = "refresh_battregs", .mode = S_IRUGO},
	. show = a6_refresh_battregs_show, .store = NULL},
};

struct a6_register_desc a6_register_desc_arr_battery_params[] = {
        [0] = { .debug_name = "TS2_I2C_BAT_RARC",
                {{.name = "getpercent", .mode = S_IRUGO},
                 .show = a6_generic_show_battery_params, .store = NULL},
                .id = {TS2_I2C_BAT_RARC}, .num_ids = 1, .ro = 1},
        [1] = { .debug_name = "TS2_I2C_BAT_RSRC",
                {{.name = "rsrc", .mode = S_IRUGO},
                 .show = a6_generic_show_battery_params, .store = NULL},
                .id = {TS2_I2C_BAT_RSRC}, .num_ids = 1, .ro = 1},
        [2] = { .debug_name = "TS2_I2C_BAT_AVG_CUR_LSB_MSB",
                {{.name = "getavgcurrent", .mode = S_IRUGO},
                 .show = a6_generic_show_battery_params, .store = NULL},
                .format = format_current,
                .id = {TS2_I2C_BAT_AVG_CUR_LSB, TS2_I2C_BAT_AVG_CUR_MSB},
                        .num_ids = 2, .ro = 1},
        [3] = { .debug_name = "TS2_I2C_BAT_TEMP_LSB_MSB",
                {{.name = "gettemp", .mode = S_IRUGO},
                 .show = a6_generic_show_battery_params, .store = NULL},
                .format = format_temp,
                .id = {TS2_I2C_BAT_TEMP_LSB, TS2_I2C_BAT_TEMP_MSB},
                        .num_ids = 2, .ro = 1},
        [4] = { .debug_name = "TS2_I2C_BAT_VOLT_LSB_MSB",
                {{.name = "getvoltage", .mode = S_IRUGO},
                 .show = a6_generic_show_battery_params, .store = NULL},
                .format = format_voltage,
                .id = {TS2_I2C_BAT_VOLT_LSB, TS2_I2C_BAT_VOLT_MSB},
                        .num_ids = 2, .ro = 1},
        [5] = { .debug_name = "TS2_I2C_BAT_CUR_LSB_MSB",
                {{.name = "getcurrent", .mode = S_IRUGO},
                 .show = a6_generic_show_battery_params, .store = NULL},
                .format = format_current,
                .id = {TS2_I2C_BAT_CUR_LSB, TS2_I2C_BAT_CUR_MSB},
                        .num_ids = 2, .ro = 1},
        [6] = { .debug_name = "TS2_I2C_BAT_COULOMB_LSB_MSB",
                {{.name = "getrawcoulomb", .mode = S_IRUGO},
                 .show = a6_generic_show_battery_params, .store = NULL},
                .format = format_rawcoulomb,
                .id = {TS2_I2C_BAT_COULOMB_LSB, TS2_I2C_BAT_COULOMB_MSB},
                        .num_ids = 2, .ro = 1},
        [7] = { .debug_name = "TS2_I2C_BAT_AS",
                {{.name = "getage", .mode = S_IRUGO|S_IWUGO},
                 .show = a6_generic_show_battery_params, .store = a6_generic_store_battery_params},
                .format = format_age,
                .id = {TS2_I2C_BAT_AS}, .num_ids = 1, .ro = 0},
        [8] = { .debug_name = "TS2_I2C_BAT_FULL_LSB_MSB",
                {{.name = "getfull", .mode = S_IRUGO},
                 .show = a6_generic_show_battery_params, .store = NULL},
                .format = format_fullx,
                .id = {TS2_I2C_BAT_FULL_LSB, TS2_I2C_BAT_FULL_MSB},
                        .num_ids = 2, .ro = 1},
        [9] = {.debug_name = "TS2_I2C_BAT_FULL40_LSB_MSB",
                {{.name = "getfull40", .mode = S_IRUGO},
                 .show = a6_generic_show_battery_params, .store = NULL},
                .format = format_fullx,
                .id = {TS2_I2C_BAT_FULL40_LSB, TS2_I2C_BAT_FULL40_MSB},
                        .num_ids = 2, .ro = 1},
        [10] = {.debug_name = "TS2_I2C_BAT_RSNSP",
                {{.name = "getrsense", .mode = S_IRUGO},
                .show = a6_generic_show_battery_params, .store = NULL},
                .format = format_rsense,
                .id = {TS2_I2C_BAT_RSNSP}, .num_ids = 1, .ro = 1},

        [11] = {.debug_name = "TS2_I2C_BAT_TEMP_LOW_LSB_MSB",
                {{.name = "temp_low", .mode = S_IRUGO|S_IWUGO},
                 .show = a6_generic_show_battery_params, .store = a6_generic_store_battery_params},
                .format = format_temp,
                .r_format = r_format_temp,
                .id = {TS2_I2C_BAT_TEMP_LOW_LSB, TS2_I2C_BAT_TEMP_LOW_MSB},
                        .num_ids = 2, .ro = 0},
        [12] = {.debug_name = "TS2_I2C_BAT_TEMP_HIGH_LSB_MSB",
                {{.name = "temp_high", .mode = S_IRUGO|S_IWUGO},
                 .show = a6_generic_show_battery_params, .store = a6_generic_store_battery_params},
                .format = format_temp,
                .r_format = r_format_temp,
                .id = {TS2_I2C_BAT_TEMP_HIGH_LSB, TS2_I2C_BAT_TEMP_HIGH_MSB},
                        .num_ids = 2, .ro = 0},
        [13] = {.debug_name = "TS2_I2C_BAT_VOLT_LOW_LSB_MSB",
                {{.name = "volt_low", .mode = S_IRUGO|S_IWUGO},
                 .show = a6_generic_show_battery_params, .store = a6_generic_store_battery_params},
                .format = format_voltage,
                .r_format = r_format_voltage,
                .id = {TS2_I2C_BAT_VOLT_LOW_LSB, TS2_I2C_BAT_VOLT_LOW_MSB},
                        .num_ids = 2, .ro = 0},
        [14] = {.debug_name = "TS2_I2C_BAT_RARC_CRIT",
                {{.name = "rarc_crit", .mode = S_IRUGO|S_IWUGO},
                 .show = a6_generic_show_battery_params, .store = a6_generic_store_battery_params},
                .id = {TS2_I2C_BAT_RARC_CRIT}, .num_ids = 1, .ro = 0},
        [15] = {.debug_name = "TS2_I2C_BAT_RARC_LOW_2",
                {{.name = "rarc_low_2", .mode = S_IRUGO|S_IWUGO},
                 .show = a6_generic_show_battery_params, .store = a6_generic_store_battery_params},
                .id = {TS2_I2C_BAT_RARC_LOW_2}, .num_ids = 1, .ro = 0},
        [16] = {.debug_name = "TS2_I2C_BAT_RARC_LOW_1",
                {{.name = "rarc_low_1", .mode = S_IRUGO|S_IWUGO},
                 .show = a6_generic_show_battery_params, .store = a6_generic_store_battery_params},
                .id = {TS2_I2C_BAT_RARC_LOW_1}, .num_ids = 1, .ro = 0},
        [17] = {.debug_name = "TS2_I2C_BAT_RAAC_MSB",
                {{.name = "getcoulomb", .mode = S_IRUGO|S_IWUGO},
                 .show = a6_generic_show_battery_params, .store = a6_generic_store_battery_params},
                .format = format_coulomb,
                .id = {TS2_I2C_BAT_RAAC_LSB, TS2_I2C_BAT_RAAC_MSB},
                        .num_ids = 2, .ro = 0},
        [18] = { .debug_name = "TS2_I2C_BAT_STATUS",
                {{.name = "status", .mode = S_IRUGO},
                .show = a6_generic_show_battery_params, .store = NULL},
                .format = format_status,
                .id = {TS2_I2C_BAT_STATUS}, .num_ids = 1, .ro = 1},
	/* special cases not corresponding to A6 hardware registers */
	[19] = {.debug_name = "VALIDATE_BATTERY",
		{{.name = "validate_battery", .mode = S_IRUGO},
		.show = a6_valid_battery_show_bp, .store = NULL}},
	[20] = {.debug_name = "BATTAUTH", 
		{{.name = "battauth", .mode = S_IRUGO | S_IWUGO},
		. show = a6_battauth_show_bp, .store = a6_battauth_store_bp}},
	...
	do {
		...
		if (rc >= 0) {
			...
			if (ai) {
				...
				if (!(*(uint32_t *)&ai->ai_waitq))
					printk(KERN_ERR "%s: ai_wq invalidated"
...
/**
 * Battery Authentication routines
 */

#define A6_WB_RETRY_MAX 20
int a6_bat_cmd_wait_busy(struct i2c_client *client, u8 *p_status)
{
	uint16_t auth_reg = TS2_I2C_BAT_COMMAND_STATUS;
	uint8_t auth_data;
	int ret, retries;

	/* Usually takes 3 retries for authentication.
	*/
	for (retries = 0; retries < A6_WB_RETRY_MAX; retries++) {
		ret = a6_i2c_read_reg(client, &auth_reg, 1, &auth_data);
		if (ret) {
			printk(KERN_ERR "%s:ERROR, ret=%d\n", __func__, ret);
			return ret ;
		}
		if (!(auth_data & TS2_I2C_BAT_STATUS_BUSY))
			break;
		msleep(10);
	}

	if (A6_WB_RETRY_MAX == retries ) {
		printk(KERN_ERR "%s ERROR: too many retries\n", __func__);
		return -1;
	}
	if (p_status)
		*p_status = auth_data;
...
/* Battery authentication code. Some pieces taken from w1_ds2784.c, other
 * parts cut-and-pasted from elsewhere in this a6.c driver.
 */

/* size of the battery challenge and response (binary). Ascii hex strings
 * are twice these.
 */
#define BATT_CHALLENGE_SIZE	8
#define BATT_RESPONSE_SIZE	20

#define BATT_OK		0
#define BATT_ERROR	-1

/* saved battery authorization state defaults to unauthorized */
static int battauth_save = BATT_ERROR;

/* Return 0 for successful battery authentication, non-zero otherwise.
 * This routine requires both the challenge and response to be provided
 * as arguments.
 * A protocol failure will fail to authenticate 
 */
static int a6_battery_authenticate(struct device *dev, const u8 *challenge, const u8 *mac)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct a6_device_state* state = i2c_get_clientdata(client);

	/* handle our equivalents to the id[] lists in a6_register_desc[] */
	static int regs_initialized = 0;
	static uint16_t write_challenge_regs[BATT_CHALLENGE_SIZE];
	static uint16_t write_mac_regs[BATT_RESPONSE_SIZE];
	uint16_t auth_reg;
	uint8_t auth_data;
	...
	if (!regs_initialized) {
		for (i = 0; i < BATT_CHALLENGE_SIZE; i++)
			write_challenge_regs[i] = TS2_I2C_BAT_CHALLENGE(i);
		for (i = 0; i < BATT_RESPONSE_SIZE; i++)
			write_mac_regs[i] = TS2_I2C_BAT_RESPONSE(i);
		regs_initialized = 1;
	}
	auth_reg = TS2_I2C_BAT_COMMAND_STATUS;
	auth_data = TS2_I2C_BAT_COMMAND_AUTH;

	/* Mutex and bootload code copied straight from a6_generic_store.
	* There are way too many copies and pastes of this code
	*/
	...
	ret = a6_i2c_write_reg(client, write_challenge_regs, BATT_CHALLENGE_SIZE,	challenge);
	if (ret < 0) {
		goto finish;
	}
	ret = a6_i2c_write_reg(client, write_mac_regs, BATT_RESPONSE_SIZE, mac);
	if (ret < 0) {
		goto finish;
	}
	ret = a6_bat_cmd_wait_busy(client, NULL);
	if (ret < 0) {
		goto finish;
	}

	/* Tell the A6 to do the authentication */
	ret = a6_i2c_write_reg(client, &auth_reg, 1, &auth_data);
	if (ret < 0) {
		goto finish;
	}

	/* read the response */
	ret = a6_bat_cmd_wait_busy(client, &auth_data);
	if (ret < 0) {
		goto finish;
	}
	if (auth_data & TS2_I2C_BAT_STATUS_AUTH_PASS) {
		ret = 0;
	}
	else
		ret = -1;
...
/* shim layers for the battery class dev type */
static ssize_t a6_valid_battery_show_bp(struct device *dev, struct device_attribute *dev_attr, char *buf)
{
	struct a6_device_state *state = (struct a6_device_state *) dev_get_drvdata(dev);
	return a6_valid_battery_show(&state->i2c_dev->dev, dev_attr, buf);
}

static ssize_t a6_battauth_show_bp(struct device *dev, struct device_attribute *dev_attr, char *buf)
{
	struct a6_device_state *state = (struct a6_device_state *) dev_get_drvdata(dev);
	return a6_battauth_show(&state->i2c_dev->dev, dev_attr, buf);
}

static ssize_t a6_battauth_store_bp(struct device *dev, struct device_attribute *dev_attr, const char *buf, size_t cnt)
{
	struct a6_device_state *state = (struct a6_device_state *) dev_get_drvdata(dev);
	return a6_battauth_store(&state->i2c_dev->dev, dev_attr, buf, cnt);
}
...
static ssize_t a6_valid_battery_show(struct device *dev, struct device_attribute *dev_attr, char *buf)
{
	...
	static u8 challenge_buf[BATT_CHALLENGE_SIZE] = { 0x74, 0xCA, 0x85, 0x99, 0x19, 0xDE, 0xD1, 0xB3 };
	static u8 mac_buf[BATT_RESPONSE_SIZE]  = { 0x87, 0xED, 0x20, 0x89, 0xAD, 0x68, 0xA2, 0xCD,
				      0x6F, 0x93, 0x13, 0x03, 0x07, 0x5A, 0x29, 0x85,
				      0xDC, 0x2E, 0xE9, 0x50 };

	res = a6_battery_authenticate(dev, challenge_buf, mac_buf);
...
/* /sys/..../battauth: battauth_store requires as input the concatentated 
 * challenge and response (must  be exactly 56 ascii characters, no '\n"); 
 * it calculates the global battauth_save. battauth_show uses this saved
 * value to output "ERROR\n" or "OK\n").
 */

static ssize_t a6_battauth_store(struct device *dev, struct device_attribute *dev_attr, const char *buf, size_t count)
{
	/* parse the buf */
	char tmp[3];	
	int i;
	u8 challenge_buf[BATT_CHALLENGE_SIZE];
	u8 response_buf[BATT_RESPONSE_SIZE];
	int res;

	/* 2 ascii hex digits per binary byte */
	int  totbuflen = 2 * (BATT_CHALLENGE_SIZE + BATT_RESPONSE_SIZE);

	if (totbuflen != strlen(buf)) {
		printk(KERN_WARNING
			"%s: Expected %d bytes, got %d\n" , __func__, totbuflen, strlen(buf) );
		return -EINVAL;
	}
	...
	for (i=0; i<BATT_RESPONSE_SIZE; i++) {
		tmp[0] = *buf++;
		tmp[1] = *buf++;
		response_buf[i] = (u8) simple_strtol(tmp, NULL, 16);
	}
	res = a6_battery_authenticate(dev,challenge_buf,response_buf);
	if (0 == res)
		battauth_save = BATT_OK;
	else
		battauth_save = BATT_ERROR;
...
static ssize_t
a6_refresh_battregs_show(
	struct device *dev,
	struct device_attribute *dev_attr,
	char *buf)
{
	int32_t ret = 0, ret_val;
	bool skip = false;
	struct i2c_client *client = to_i2c_client(dev);
	struct a6_device_state *state = i2c_get_clientdata(client);
	uint16_t cmdstatus_reg = TS2_I2C_BAT_COMMAND_STATUS;
	uint8_t cmdstatus_val = TS2_I2C_BAT_COMMAND_REFRESH;
	...
	while (test_and_set_bit(DEVICE_BUSY_BIT, state->flags)) {
		...
		/* busy bit set? wait to be cleared (at least
		   5 minutes: in jiffies) */
		ret = wait_event_interruptible_timeout(state->dev_busyq,
				!test_bit(DEVICE_BUSY_BIT, state->flags),
				300*HZ);
		if (!ret) {
			printk(KERN_ERR "%s: wait on device busy"
	...
	if (test_bit(BATTREGS_REFRESH_INITIATED, state->flags)) {
		skip = true;
	} else {
		ret_val = test_and_set_bit(BOOTLOAD_ACTIVE_BIT,
			state->flags);
		ASSERT(!ret_val);
		ret_val = test_and_set_bit(BATTREGS_REFRESH_INITIATED,
			state->flags);
		ASSERT(!ret_val);
	}
	...
	/* refresh regs... */
	ret = a6_i2c_write_reg(client, &cmdstatus_reg, 1, &cmdstatus_val);
	if (ret < 0)
		goto exit0;

	/* read the response */
	ret = a6_bat_cmd_wait_busy(client, &cmdstatus_val);
	if (ret < 0)
		goto exit0;
	if (!(cmdstatus_val & TS2_I2C_BAT_STATUS_REGS_VALID))
		ret = -1;
...
exit0:
	...
	if (false == skip) {
		if (test_bit(BATTREGS_REFRESH_INITIATED, state->flags))
			clear_bit(BATTREGS_REFRESH_INITIATED, state->flags);
		if (test_bit(BOOTLOAD_ACTIVE_BIT, state->flags))
	...
	if (true == skip)
		snprintf(buf, PAGE_SIZE, "SKIPPING...\n");
	else if (!ret)
		snprintf(buf, PAGE_SIZE, "OK\n");
	else
...
static ssize_t a6_generic_show_battery_params(struct device *dev, struct device_attribute *attr, char *buf)
{
        struct a6_device_state *state = (struct a6_device_state *) dev_get_drvdata(dev);
	return a6_generic_show(&state->i2c_dev->dev, attr, buf);
}
...
static ssize_t a6_generic_store_battery_params(struct device *dev, struct device_attribute *attr, const char *buf,
                                size_t count)
{
	struct a6_device_state *state = (struct a6_device_state *) dev_get_drvdata(dev);
	return a6_generic_store(&state->i2c_dev->dev, attr, buf, count);
}
...
int32_t a6_create_battery_params_dev_files(struct a6_device_state* state)
{
	int32_t idx;
	int32_t rc = 0;
	struct class *batt_class;
	struct device *batt_class_dev;

        /* Re-create battery query/control entries in "/sys/class/battery/batt0" */
        batt_class = class_create(THIS_MODULE, "battery");
        if (!batt_class){
                printk(KERN_ERR "Unable to create battery class node\n");
                return -1;
        }

        batt_class_dev = device_create(batt_class, 0, 0, state, "batt0");
        if (!batt_class_dev) {
                printk(KERN_ERR "Unable to create battery class device node\n");
		rc = -1;
                goto err0;
        }

	for (idx = 0; idx < ARRAY_SIZE(a6_register_desc_arr_battery_params); idx++) {
		rc = device_create_file(batt_class_dev, &a6_register_desc_arr_battery_params[idx].dev_attr);
		if (rc < 0) {
			printk(KERN_ERR "%s: failed to create dev_attr file for %s.\n",
			       __func__, a6_register_desc_arr[idx].dev_attr.attr.name);
			goto err1;
...
err1:
	// error: cleanup files already created.
	for (idx--; idx >= 0; idx--) {
		device_remove_file(batt_class_dev, &a6_register_desc_arr[idx].dev_attr);
	}
	
err0:
	class_destroy(batt_class);
	return rc;
}
...
static int a6_i2c_probe(struct i2c_client *client, const struct i2c_device_id *dev_id)
{
	...
	rc = a6_create_battery_params_dev_files(state);
	if (rc < 0) {
		goto err7;
	}
