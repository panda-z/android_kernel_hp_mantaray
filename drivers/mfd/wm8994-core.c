		     ...
		     int count, u16 *buf)
{
	...
	while (count > 127)
	{
		ret = wm8994_read(wm8994, reg, 127*2, buf);
		count = count - 127;
		buf = buf + 127;
		reg = reg + 127;
	}
		      ...
		      int count, const u16 *buf)
{
	...
	while (count > 127)
	{
		ret = wm8994_write(wm8994, reg, 127*2, buf);
		count = count - 127;
		buf = buf + 127;
		reg = reg + 127;
	}
...
static int wm8994_device_suspend(struct device *dev)
{
	...
	if (pdata && pdata->force_route) {
		dev_info(dev, "Codec route is forced, do not suspend");
		return 0;
	}
...
static int wm8994_device_resume(struct device *dev)
{
	...
	if (pdata && pdata->force_route) {
		dev_info(dev, "Codec route is forced, do not resume");
		return 0;
	}

	if (pdata->wm8994_setup && !pdata->force_route)
		pdata->wm8994_setup();
	...
	for (i = 0; i < ARRAY_SIZE(wm8994->ldo_regs); i++) {
		if (wm8994->ldo_regs[i]) {
	...
	for (i = 0; i < ARRAY_SIZE(wm8994->gpio_regs); i++) {
		if (wm8994->gpio_regs[i]) {
...
retry:
	...
	if (ret < 0) {
		dev_err(wm8994->dev, "Failed to read ID register reg id = 0x%x\n",ret);
		if (retries-- > 0) {
			msleep(100);
			goto retry;
		} else
	...
	default:
	 ...
	 dev_err(wm8994->dev, "Audio Codec Device ID: %x\n",
                                 ret);
